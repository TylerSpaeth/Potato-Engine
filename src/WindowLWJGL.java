...
public class WindowLWJGL {
  ...
  // NOTICE this class is not currently being used
  ...
  private static WindowLWJGL _window = null;
   ...
   */
  public WindowLWJGL(int width, int height, String title) {
   ...
   */
  public WindowLWJGL(int width, int height, String title, boolean vsync) {
    // If another window already exist throw an exception. This way only one can exist
    if(_window != null) {
      throw new IllegalStateException("A window has already been initialized, only one may exist" +
          " at a time ");
